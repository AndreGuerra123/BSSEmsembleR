...
#* @filter tokenizer
tokenizer <-function(req){
  body<-jsonlite::fromJSON(req$postBody)
  getTokenValidation(body)
  plumber::forward()
}
...
getLoginValidation<-function(body){
      ...
      testit::assert('Invalid username.',{isValidString(body$username)})
      testit::assert('Invalid password.',{isValidString(body$password)})
      user <- getUserByUsername(body$username)
      testit::assert('Invalid username.',{length(user)!=0})
      testit::assert('Failed to retrieve hash.',{is.character(user$hash[[1]])})
      p<-as.character(body$password)
      h<-as.character(user$hash[[1]])
      testit::assert('Invalid password.', {bcrypt::checkpw(p,h)})
...
getRegistrationValidation<-function(body){
    ...
    testit::assert('Invalid username.',{isValidString(body$username)})
    testit::assert('Invalid password.',{isValidString(body$password)})
    testit::assert('Invalid password confirmation.',{isValidString(body$validation)})
    testit::assert('Invalid password confirmation.',{body$password==body$validation})
    testit::assert('Invalid invitation key.',{body$invitation==.GlobalEnv$BSSEInvitation})
    user <- getUserByUsername(body$username)
    testit::assert('Invalid username.',{length(user)==0})
...
isValidString<-function(x){
  if(!isvar(x) || is.null(x) || is.na(x) || x==''){
    return(FALSE)
  }else{
    return(TRUE)
  }
...
createUser<-function(username,password){
  ...
  return(list("id"=newid,"hash"=newhash))
}
